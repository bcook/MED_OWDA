%function  [s, c, ph, ci, phi, phu] = cmtm(x,y,dt,NW,qbias,confn,qplot);
%
...
function [s, c, ph, ci, phi, phu, phl] = cmtm(x,y,dt,NW,qbias,confn,qplot);
...
ci(1,:)=cohconf(v,.95) .* ones(1,size(c,2));  %not corrected for bias, this is conservative.
ci(2,:)=cohconf(v,.90) .* ones(1,size(c,2));
%not corrected for bias, this is conservative.
% ci=ci.*ones(2,size(c,2));
